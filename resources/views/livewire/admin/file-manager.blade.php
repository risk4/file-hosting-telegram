<div class="space-y-5">
    ...
    <div class="flex flex-wrap gap-2">
        ...
        <button wire:click="$set('type','{{ $t['v'] }}')"
                class="px-3 py-1 text-xs font-mono rounded-lg border transition-all
                       {{ $type === $t['v'] ? 'bg-teal-500/15 border-teal-500/40 text-teal-400' : 'bg-gray-900 border-gray-800 text-gray-500 hover:border-gray-600' }}">
            {{ $t['l'] }} ({{ $counts[$t['k']] }})
        </button>
        @endforeach
    </div>
                    ...
                    <tr class="hover:bg-gray-800/40 transition-colors group">
                        <td class="px-5 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <div class="text-xs text-gray-500 font-mono truncate max-w-xs">{{ $file->original_name }}</div>
                                </div>
                                @if(!$file->is_public)
                        ...
                        <td class="px-5 py-3">
                            <div class="flex items-center justify-end gap-1">
                                @if($file->type === 'note')
                                <button wire:click="viewNote({{ $file->id }})"
                                ...
                                <button wire:click="openEdit({{ $file->id }})"
    ...
    {{-- Note View Modal --}}
    @if($showNoteView && $viewingNote)
    <div class="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-800">
                <h3 class="font-semibold">{{ $viewingNote->name }}</h3>
                <button wire:click="$set('showNoteView', false)" class="text-gray-500 hover:text-white">✕</button>
            </div>
            <div class="flex-1 overflow-auto p-6">
                <pre class="bg-gray-950 rounded-xl p-4 text-sm font-mono text-teal-300 whitespace-pre-wrap break-words">{{ $viewingNote->content }}</pre>
            </div>
            <div class="px-6 py-4 border-t border-gray-800">
                <button onclick="navigator.clipboard.writeText(`{{ addslashes($viewingNote->content ?? '') }}`)"
